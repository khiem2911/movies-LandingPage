...
const TopNews:React.FC<topnews> = (props) => {
  ...
  if (data) {
    content = data.map((item) => {
        ...
        const convertDate = publishedDate.toLocaleString('en-US',{month:'long',day:'numeric',year:'numeric'})
  ...
  return (
    <div className={classes.container_topNews}>
      <div className={classes.header_topNews}>
        <h2>Top News</h2>
        <Link to="/">See more</Link>
      </div>
      <div style={styles} >
        <div className={classes.custom_youtube}>
        <YouTube opts={opts} videoId="k397HRbTtWI" />
        </div>
